...
class MAE_UNet(nn.Module):
    ...
    def forward(self, x, building_mask, min_samples, max_samples, pre_sampled=False, method='linear'):
      ...
      sample_mask = (x[1] != 1).to(torch.float32)
      map1 = torch.tensor([interpolate_map(sm.numpy(), method) for sm in x]).unsqueeze(1)
    ...
    def step(self, batch, optimizer, min_samples, max_samples, train=True, free_space_only=False, mae_regularization=False, method='linear'):
        with torch.set_grad_enabled(train):
            ...
            map1, _, pred_map = self.forward(complete_map, building_mask, min_samples, max_samples, method=method).to(torch.float32)
    ...
    def fit(self, train_dl, test_dl, optimizer, scheduler, min_samples, max_samples, dB_max=-47.84, dB_min=-147,
            free_space_only=False, epochs=100, save_model_epochs=25, eval_model_epochs=5, save_model_dir ='/content', mae_regularization=False, method='linear'):
        for epoch in range(epochs):
            self.train()
            running_loss = 0.0
            for i, batch in enumerate(train_dl):
                loss = self.step(batch, optimizer, min_samples, max_samples, train=True, free_space_only=free_space_only, mae_regularization=mae_regularization, method=method)
    ...
    def fit_wandb(self, train_dl, test_dl, optimizer, scheduler, min_samples, max_samples, project_name, run_name, 
                  dB_max=-47.84, dB_min=-147, free_space_only=False, epochs=100, save_model_epochs=25, save_model_dir='/content', mae_regularization=False, method='linear'):
        import wandb
    ...
    def evaluate(self, test_dl, min_samples, max_samples, dB_max=-47.84, dB_min=-147, free_space_only=False, method='linear', pre_sampled=False):
        self.eval()
        losses = 0
        pixels = 0
        with torch.no_grad():
            for i, batch in enumerate(test_dl):
                    ...
                    if pre_sampled:
                        ...
                        _, _, pred_map = self.forward(sampled_map, building_mask, min_samples, max_samples, method=method, pre_sampled=True)
                        pred_map = pred_map.detach().cpu()

                    else:
                        complete_map = complete_map.to(torch.float32).to(device)
                        _, _, pred_map = self.forward(complete_map, building_mask, min_samples, max_samples, method=method, pre_sampled=False)
                        pred_map = pred_map.detach().cpu()
