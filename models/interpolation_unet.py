...
class Interpolation_UNet(nn.Module):
    def __init__(self,
                 model_name,
                 model_type='Interpolation_UNet',
        ...
        super(Interpolation_UNet, self).__init__()
    ...
    def step(self, batch, optimizer, min_samples, max_samples, method, train=True, free_space_only=False):
        with torch.set_grad_enabled(train):
            ...
            map1, _, pred_map = self.forward(complete_map, building_mask, min_samples, max_samples, method=method)
    ...
    def fit(self, train_dl, val_dl, optimizer, scheduler, min_samples, max_samples, method, run_name=None, dB_max=-47.84,
             dB_min=-147, free_space_only=False, epochs=100, save_model_epochs=25, eval_model_epochs=5, save_model_dir ='/content'):
        
        if run_name is None:
            run_name = f'{min_samples}-{max_samples} samples, {method} interpolation'
            if free_space_only:
        ...
        self.training_config = dict(train_batch=train_dl.batch_size, val_batch=val_dl.batch_size, min_samples=min_samples,
                                    max_samples=max_samples, method=method, run_name=run_name, dB_max=dB_max, dB_min=dB_min, 
        ...
        for epoch in range(epochs):
            ...
            for i, batch in enumerate(train_dl):
                loss = self.step(batch, optimizer, min_samples, max_samples, method, train=True, free_space_only=free_space_only)
                running_loss += loss.detach().item()
                print(f'{loss}, [{epoch + 1}, {i + 1:5d}] loss: {running_loss/(i+1)}')

            if eval_model_epochs > 0:
                if (epoch + 1) % eval_model_epochs == 0:
                    test_loss = self.evaluate(val_dl, min_samples, max_samples, method, dB_max, dB_min, free_space_only=free_space_only)
    ...
    def fit_wandb(self, train_dl, val_dl, optimizer, scheduler, min_samples, max_samples, method, project_name, run_name=None, 
        ...
        self.training_config = dict(train_batch=train_dl.batch_size, val_batch=val_dl.batch_size, min_samples=min_samples,
                                    max_samples=max_samples, method=method, project_name=project_name, run_name=run_name,
                                    dB_max=dB_max, dB_min=dB_min, free_space_only=free_space_only)
    ...
    def evaluate(self, test_dl, min_samples, max_samples, method, dB_max=-47.84, dB_min=-147, free_space_only=False, pre_sampled=False):
        self.eval()
        losses = 0
        pixels = 0
        with torch.no_grad():
            for i, batch in enumerate(test_dl):
                    ...
                    if pre_sampled:
                        ...
                        _, _, pred_map = self.forward(sampled_map, building_mask, min_samples, max_samples, method, pre_sampled=True)
                        pred_map = pred_map.detach().cpu()

                    else:
                        complete_map = complete_map.to(torch.float32).to(device)
                        _, _, pred_map = self.forward(complete_map, building_mask, min_samples, max_samples, method, pre_sampled=False)
                        pred_map = pred_map.detach().cpu()
    ...
    def visualize_maps(self, x, building_mask, min_samples, max_samples, method, pre_sampled):
        self.eval()
        map1, sample_mask, map2 = self.forward(x, building_mask, min_samples, max_samples, method, pre_sampled)
        if pre_sampled:
            x = x[:,0]
